...
function OpenAmStrategy(options, verify) {
  ...
  if (!options.callbackUrl) throw new Error('OpenAmStrategy requires a callbackUrl option');
	_openAmRealm = (options.openAmRealm === undefined) ? "/" : options.openAmRealm;
	_openAmCookieName = (options.openAmCookieName === undefined) ? "iPlanetDirectoryPro" : options.openAmCookieName;

	this._openAm = new OpenAm(options.openAmBaseUrl, _openAmRealm, _openAmCookieName);
...
OpenAmStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.headers.cookie) {
    ...
    if (cookies[this._openAmCookieName]) {
        ...
        this._openam.isTokenValid(token, function(err, data){
          if (err) { return self.error(new InternalOpenAmError('failed to validate toke', err));}

          if (data){
            self._loadUserProfile(token, function(err, data){
